...
async function parseModelJson(response) {
    ...
    if (!text) {
        throw createHttpError(502, "AI returned an empty response")
    }

    const sanitized = text.replace(/^```(?:json)?\s*/i, "").replace(/```$/i, "").trim()

    try {
        return JSON.parse(sanitized)
    } catch (error) {
        throw createHttpError(502, "AI returned invalid JSON")
    }
}
...
`
    ...
    throw createHttpError(502, "Unable to generate interview report after retries and model fallback")

}

async function generatePdfFromHtml(htmlContent) {
    const isProduction = process.env.NODE_ENV === 'production'
    const puppeteer = isProduction ? null : require('puppeteer')
    const puppeteerCore = isProduction ? require('puppeteer-core') : null
...
async function generateResumePdf({ resume, selfDescription, jobDescription }) {
    ...
    const parsed = resumePdfSchema.safeParse(jsonContent)
    if (!parsed.success) {
        throw createHttpError(422, "AI returned incomplete resume content", parsed.error.issues)
    }
